...
export class GamepadExecutor implements InputExecutor {
  ...
  private updateGamepadState(newState: Set<string>): void {
    // 找出新增的按钮（需要按下）
    const buttonsToPress = new Set([...newState].filter(button => !this.currentGamepadState.has(button)));

    // 找出移除的按钮（需要释放）
    const buttonsToRelease = new Set([...this.currentGamepadState].filter(button => !newState.has(button)));

    // 只在状态有变化时记录日志
    if (buttonsToPress.size > 0 || buttonsToRelease.size > 0) {
      // 先释放需要释放的按钮
      if (buttonsToRelease.size > 0) {
        console.log('GamepadEvent: Releasing buttons:', Array.from(buttonsToRelease));

        try {
          keySender.releaseKey(Array.from(buttonsToRelease));
        } catch (error) {
          console.error('GamepadError: Error releasing buttons:', error);
        }
      }
      ...
      if (buttonsToPress.size > 0) {
        ...
        console.log('GamepadEvent: Pressing buttons:', buttonsToPressAll);
        ...
        try {
          ...
          console.error('GamepadError: Error pressing buttons:', error);
        }
      }
